...
import malignancy_classification_model.dsets
...
class malignancyClassificationTrainingApp:
    ...
    def initModel(self):
        # The code fine-tunes the model by randomly initializing the head layer only, and updating it.
        #  updating the last conv block, and freezing all other layers to retain pretrained features. 

        model_cls = getattr(malignancy_classification_model.model, self.cli_args.model)
        model = model_cls()

        if self.cli_args.finetune:
            d = torch.load(self.cli_args.finetune, map_location='cpu')
            model_blocks = [
                n for n, subm in model.named_children()   # Filters out top-level modules that have parameters (as opposed to the final activation0) :  selecting only the main building blocks of the model that contain trainable parameters.
                if len(list(subm.parameters())) > 0
            ]
            finetune_blocks = model_blocks[-self.cli_args.finetune_depth:] # Takes the last finetune_depth blocks. The default (if fine-tuning) is 1. if depth=2 : I get ['block4', 'head_linear']
            ...
            # Starting from a fully initialized model would have us begin with (almost) all nodules labeled as
            # malignant, because that output means “nodule” in the classifier we start from.
            # d['model_state']: It contains weight tensors mapped to their parameter names (e.g., block1.conv1.weight, block4.conv2.bias, head_linear.weight, etc.).
            # model_blocks[-1] refers to the last block in the model (head_linear)
            # Splits each key at the first dot (.) to get the top-level module name and then Excludes any parameters where the top-level module matches head_linear.
            model.load_state_dict(
                {
                    k: v for k,v in d['model_state'].items() 
                    if k.split('.')[0] not in model_blocks[-1] # Initlized randomly the head layer only
                },
                strict=False, #Passing strict=False lets us load only some weights of the module (with the filtered ones missing).
            )

            for n, p in model.named_parameters():
                if n.split('.')[0] not in finetune_blocks: # We want gradients for finetune_blocks only (['block4', 'head_linear'])
    ...
    def initTrainDl(self):
        ds_cls = getattr(malignancy_classification_model.dsets, self.cli_args.dataset)
    ...
    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g,
        ...
        if augment:
            input_g = malignancy_classification_model.model.augment3d(input_g)
...
if __name__ == '__main__':
    malignancyClassificationTrainingApp().main()
